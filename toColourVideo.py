...
while(cap.isOpened()):
	...
	colorized = cv2.resize(colorized,(img.shape[1], img.shape[0]), interpolation=cv2.INTER_CUBIC)

	#result = cv2.hconcat([img,colorized])
	#cv2.imshow("Grayscale -> Colour", colorized)

	i = i + 1
	# Save the image
	cv2.imwrite('output/coloured%d.png'%i, colorized)
